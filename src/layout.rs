...
use thiserror::Error;
use crate::monitor::{MonitorTracker, MonitorInfo};
...
pub struct DefaultLayout {
    ...
    #[serde(default = "MonitorTracker::primary_monitor_connector")]
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Clone, PartialEq)]
pub struct FinalizedLayout(pub Vec<DisplayConfiguration>);
...
pub fn finalize_layout(monitors: Vec<MonitorInfo>, layout: Layout) -> Result<FinalizedLayout, LayoutError> {
    match layout {
        Layout::Default(layout) => {
            let primary_monitor = layout
                .primary_monitor
                .filter(|name| validate_monitor_exists(&monitors, name))
                .or_else(MonitorTracker::primary_monitor_connector);
